...
2.  **ドメインモデル (Domain Model):**
    * **責務:** `Nekote.Core.AI.Domain` の純粋な内部ビジネス概念(例: `Conversation`)。
...
`Infrastructure` レイヤー(例: `Nekote.Core.AI.Infrastructure.OpenAI`)に、外部 API の仕様と 1:1 で対応する DTO クラス群を定義する。これらは `internal` としてカプセル化し、`Domain` レイヤーから参照できないようにすることが推奨される。
...
namespace Nekote.Core.AI.Infrastructure.OpenAI.Dtos
{
    ...
    internal class OpenAiContentPartDto
    {
        ...
        // "image_url" DTO は Nekote.Core.AI のスコープ外のため、
        // デシリアライズ対象に含めない。
    }
    ...
    internal class OpenAiContentConverter : JsonConverter<OpenAiContentBase>
    {
            ...
            JsonSerializerOptions options)
        {
            switch (value)
            {
                // Nekote.Core.AI はテキストリクエストのみを送信するため、
...
同様に、Gemini の API 仕様に対応する DTO も `Nekote.Core.AI.Infrastructure.Gemini.Dtos` 名前空間に定義する。これらは OpenAI の DTO とは**全く異なる構造**を持つことになる(例: `contents` と `parts` の階層、`role` が `model` と `user` のみ)。
...
これらは `Nekote.Core.AI.Domain` レイヤーに配置される、純粋な POCO (Plain Old C# Object) である。これらは外部 API の仕様やシリアライズ属性を**一切含まない**。

```csharp
namespace Nekote.Core.AI.Domain
{
    /// <summary>
    /// Nekote.Core.AI が内部で統一的に使用する、
    /// プロバイダから独立したチャットロール。
    /// </summary>
    ...
    /// <summary>
    /// Nekote.Core.AI の純粋なチャットメッセージ・ドメインモデル。
...
namespace Nekote.Core.AI.Domain.Contracts
{
    /// <summary>
...
`Nekote.Core.AI.Infrastructure.OpenAI` プロジェクト内に配置される。

```csharp
using Microsoft.Extensions.Configuration;
using Nekote.Core.AI.Domain;
using Nekote.Core.AI.Domain.Contracts;
using Nekote.Core.AI.Infrastructure.OpenAI.Dtos; // 内部 DTO を参照
...
`Nekote.Core.AI.Infrastructure.Gemini` プロジェクト内に配置される。
OpenAI のリポジトリと**同じドメイン契約**(`IChatCompletionService`)を実装するが、**全く異なる内部 DTO とマッピングロジック**を使用することが重要である。

```csharp
using Nekote.Core.AI.Domain;
using Nekote.Core.AI.Domain.Contracts;
using Nekote.Core.AI.Infrastructure.Gemini.Dtos; // Gemini 固有の DTO を参照
...
namespace Nekote.Core.AI.Infrastructure.Gemini
{
    ...
    internal class GeminiChatRepository : IChatCompletionService
    {
        ...
        private GeminiChatRequestDto MapToGeminiRequestDto(
            IEnumerable<NekoteChatMessage> history)
        {
            // AI はここに、Nekote.Core.AI の履歴 (System ロールを含む可能性がある) を、
