...
2.  **ドメインモデル (Domain Model):**
    * **責務:** `Nekote.Core` の純粋な内部ビジネス概念（例: `Conversation`）。
...
`Infrastructure` レイヤー（例: `Nekote.Infrastructure.OpenAI`）に、外部 API の仕様と 1:1 で対応する DTO クラス群を定義する。これらは `internal` としてカプセル化し、`Domain` レイヤーから参照できないようにすることが推奨される。
...
namespace Nekote.Infrastructure.OpenAI.Dtos
{
    /// <summary>
    ...
    internal class OpenAiChatRequestDto
    {
        ...
        public bool Stream { get; set; } = false; // 今回はストリーミングをサポートしない

        // (temperature, max_tokens 等、他のプロパティは必要に応じて追加)
    }
    ...
    internal class OpenAiMessageDto
    {
        ...
        /// (Nekote.Core はマルチモーダル入力をサポートしないため)
...
namespace Nekote.Infrastructure.OpenAI.Dtos
{
    ...
    internal class OpenAiChoiceDto
    {
        ...
        public OpenAiMessageDto Message { get; set; } // 応答メッセージ
...
namespace Nekote.Infrastructure.OpenAI.Dtos
{
    ...
    {
        public List<OpenAiContentPartDto> Parts { get; set; } = new();
    }
    ...
    internal class OpenAiContentPartDto
    {
        ...
        // "image_url" DTO は Nekote.Core のスコープ外のため、
        // デシリアライズ対象に含めない。
    }
    ...
    internal class OpenAiContentConverter : JsonConverter<OpenAiContentBase>
    {
            ...
            JsonSerializerOptions options)
        {
            switch (value)
            {
                // Nekote.Core はテキストリクエストのみを送信するため、
...
同様に、Gemini の API 仕様に対応する DTO も `Nekote.Infrastructure.Gemini.Dtos` 名前空間に定義する。これらは OpenAI の DTO とは**全く異なる構造**を持つことになる（例: `contents` と `parts` の階層、`role` が `model` と `user` のみ）。

```csharp
namespace Nekote.Infrastructure.Gemini.Dtos
{
    // (Gemini API の仕様に基づき、
...
これらは `Nekote.Core` の `Domain` レイヤーに配置される、純粋な POCO (Plain Old C# Object) である。これらは外部 API の仕様やシリアライズ属性を**一切含まない**。

```csharp
namespace Nekote.Core.Domain.Ai
{
    /// <summary>
    /// Nekote.Core が内部で統一的に使用する、
    /// プロバイダから独立したチャットロール。
    /// </summary>
    ...
    /// <summary>
    /// Nekote.Core の純粋なチャットメッセージ・ドメインモデル。
...
namespace Nekote.Core.Domain.Ai.Contracts
{
    /// <summary>
...
`Nekote.Infrastructure.OpenAI` プロジェクト内に配置される。

```csharp
using Microsoft.Extensions.Configuration;
using Nekote.Core.Domain.Ai;
using Nekote.Core.Domain.Ai.Contracts;
using Nekote.Infrastructure.OpenAI.Dtos; // 内部 DTO を参照
using System.Net.Http; // HttpClient を使用
using System.Text.Json; // JsonSerializer を使用

namespace Nekote.Infrastructure.OpenAI
{
    /// <summary>
    ...
    internal class OpenAiChatRepository : IChatCompletionService
    {
            ...
            CancellationToken ct)
        {
            ...
            throw new NotImplementedException(
                "AI はここに HttpClient POST リクエストと JSON デシリアライズ処理を実装する必要があります。");
        ...
        private string ExtractTextFromContent(OpenAiContentBase content)
        {
            ...
            throw new NotImplementedException(
                "AI はここに OpenAiContentBase の具象型 (String/Array) に基づくテキスト抽出ロジックを実装する必要があります。");
        }
...
`Nekote.Infrastructure.Gemini` プロジェクト内に配置される。
OpenAI のリポジトリと**同じドメイン契約**（`IChatCompletionService`）を実装するが、**全く異なる内部 DTO とマッピングロジック**を使用することが重要である。

```csharp
using Nekote.Core.Domain.Ai;
using Nekote.Core.Domain.Ai.Contracts;
using Nekote.Infrastructure.Gemini.Dtos; // Gemini 固有の DTO を参照

namespace Nekote.Infrastructure.Gemini
{
    /// <summary>
    ...
    internal class GeminiChatRepository : IChatCompletionService
    {
            ...
            CancellationToken ct)
        {
            ...
            // (このスケルトンでは NotImplementedException を throw する)
            throw new NotImplementedException(
                "AI はここに、Gemini 固有の DTO (GeminiChatRequestDto) へのマッピング、API 呼び出し、およびレスポンス DTO からドメインへの再マッピング処理を実装する必要があります。");
        ...
        private GeminiChatRequestDto MapToGeminiRequestDto(
            IEnumerable<NekoteChatMessage> history)
        {
            // AI はここに、Nekote.Core の履歴 (System ロールを含む可能性がある) を、
            // Gemini の "user"/"model" の交互の履歴に変換するロジックを実装する。
            ...
            GeminiChatResponseDto responseDto)
        {
            ...
            // テキストを抽出し、NekoteChatMessage を生成するロジックを実装する。
